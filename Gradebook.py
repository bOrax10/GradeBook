...
def change_password():
    ...
    if currentPassword == details[2] and newPassword1 == newPassword2:
        ...
        user_df.to_csv('path_to_folder/user.csv', index=False)
...
def signup():
    ...
    if class_entry2.get() not in os.listdir('path_to_folder'):
        os.mkdir('path_to_folder/' + class_entry2.get())  # making a new directory/folder.
...
def create_file():
    ...
    path = "path_to_folder/" + details[1]

    if file_name not in os.listdir(path):
        df = pd.DataFrame(columns=['Name', 'RollNo', 'Maths', 'Physics', 'Chemistry', 'IP', 'English'])
        df.to_csv("path_to_folder/" + details[1] + '/' + file_name + '.csv', index=False)
...
user_df = pd.read_csv('path_to_folder/user.csv', index_col=None)

data = None

bg_img = Image.open('path_to_folder/bg.png')
bg_img = bg_img.resize((960, 1080))
bg = ImageTk.PhotoImage(bg_img)
bg_label = tk.Label(main_frame, image=bg)

show_password_img = Image.open('path_to_folder/show.png')
show_password_img = show_password_img.resize((30, 30))
show_password_img = ImageTk.PhotoImage(show_password_img)

hide_password_img = Image.open('path_to_folder/hide.png')
...
logo_img = Image.open('path_to_folder/logo.png')
logo_img = logo_img.resize((256, 250))
logo = ImageTk.PhotoImage(logo_img)
logo_label = tk.Label(main_frame, image=logo)

login_button_img = Image.open('path_to_folder/log.png')
login_button_img = login_button_img.resize((250, 65))
login_button_img = ImageTk.PhotoImage(login_button_img)

signup_button_img = Image.open('path_to_folder/sig.png')
signup_button_img = signup_button_img.resize((250, 65))
signup_button_img = ImageTk.PhotoImage(signup_button_img)

go_back_img = Image.open('path_to_folder/goback.png')
go_back_img = go_back_img.resize((45, 45))
go_back_img = ImageTk.PhotoImage(go_back_img)

logout_img = Image.open('path_to_folder/logout.png')
logout_img = logout_img.resize((250, 65))
logout_img = ImageTk.PhotoImage(logout_img)

school_name_label = tk.Label(main_frame, text='D.A.V Public School', font=('Constantia', 30))
school_name_label.place(relx=0.085, rely=0.5)
gradeBook_label = tk.Label(main_frame, text='GRADEBOOK', font=('Copperplate Gothic Light', 35))
gradeBook_label.place(relx=0.085, rely=0.6)
...
logo_label.place(relx=0.115, rely=0.2)
